(function installServoForge3DBottleLabelViewport(global) {
  "use strict";

  const VIEWPORT_VERSION = "servoforge.3d-viewport.v0.7";
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .servoforge-3d-telemetry{position:absolute;left:14px;bottom:14px;display:grid;grid-template-columns:repeat(5,minmax(112px,1fr));gap:8px;width:min(1130px,calc(100% - 28px));pointer-events:none}.servoforge-3d-metric{background:rgba(5,15,20,.85);border:1px solid rgba(137,174,190,.2);border-radius:10px;padding:8px 9px;box-shadow:0 8px 24px rgba(0,0,0,.2);min-width:0}.servoforge-3d-metric span{display:block;color:#829ba7;font-size:9px;text-transform:uppercase;letter-spacing:.08em}.servoforge-3d-metric strong{display:block;margin-top:3px;font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.servoforge-3d-metric.action{grid-column:span 2}.servoforge-3d-metric.physical strong{color:#b9f4d8}.servoforge-3d-metric.live strong{color:#ffad8f}.servoforge-3d-metric.map strong{color:#b9ddff}.servoforge-3d-metric.pad strong{color:#efb479}.servoforge-3d-metric.spender strong{color:#e4e9eb}.servoforge-3d-metric.label strong{color:#f5f0d8}
      .servoforge-3d-note{position:absolute;right:14px;top:14px;max-width:450px;background:rgba(5,15,20,.82);border:1px solid rgba(137,174,190,.16);border-radius:9px;padding:8px 10px;color:#9db2bb;font-size:10px;line-height:1.42;pointer-events:none}.servoforge-3d-note strong{color:#ffd1c1}
  ...
  function installUi() {
    ...
    backdrop.innerHTML = `
      <section class="servoforge-3d-panel" role="dialog" aria-modal="true" aria-labelledby="servoforge3dTitle">
        <header class="servoforge-3d-head">
          <div class="servoforge-3d-title"><span class="servoforge-3d-live" aria-hidden="true"></span><div><h2 id="servoforge3dTitle">ServoForge 3D • Bottle Labels</h2><small>Active recipe label wraps • spender plates • measured wipe pads • sensors hidden • read only • Three.js ${THREE_VERSION}</small></div></div>
        ...
        <div class="servoforge-3d-stage" id="servoforge3dStage">
          <canvas class="servoforge-3d-canvas" id="servoforge3dCanvas" aria-label="Live ServoForge 3D bottle-label, spender-plate, wipe-pad, and bottle-table simulation"></canvas>
          <div class="servoforge-3d-note"><strong>Label authority:</strong> neck/body/back wrap angles come from the active ServoForge label specification. Neck height uses the stored neck-height value. Body/back label heights and all artwork remain reference-only until measured label dimensions/artwork are supplied. Labels appear after their application aggregate is passed.</div>
          <div class="servoforge-3d-legend"><b>3D equipment</b><br>Cream: applied bottle labels<br>Orange: live Head 1<br>Silver: application arm / spender plate<br>Brown: measured wipe sponge<br>Steel strip: wipe backing<br>Blue: wipe rollers<br>Purple: coder</div>
          <div class="servoforge-3d-telemetry" aria-live="polite">
            ...
            <div class="servoforge-3d-metric label"><span>Applied labels</span><strong id="servoforge3dLabels">—</strong></div>
    ...
    ui = {
      ...
      labels: backdrop.querySelector("#servoforge3dLabels"),
  ...
  function createBottleModel(geometry, labels) {
    ...
    const markerHeight = Math.max(0.18, Math.min(height * 0.32, number(geometry?.bottle?.bodyStraightHeightMm, 92) * unitsPerMm * 0.64));
    const markerThickness = Math.max(0.009, bodyRadius * 0.07);
    const marker = new THREE.Mesh(
      new THREE.BoxGeometry(markerThickness, markerHeight, Math.max(0.022, bodyRadius * 0.20)),
      new THREE.MeshStandardMaterial({ color: 0xff6a3d, emissive: 0x421308, emissiveIntensity: 0.35, roughness: 0.35 })
    );
    marker.position.set(bodyRadius + markerThickness * 0.45, Math.max(markerHeight / 2 + 0.04, height * 0.19), 0);
    group.add(marker);

    const labelGroup = labelMeshFactory()?.createBottleLabels?.(THREE, labels, geometry);
    if (labelGroup) {
      group.add(labelGroup);
      group.userData.labelMeshes = labelGroup.userData.labelMeshes || {};
      group.userData.labelBrand = labels?.brand || "";
    }
    return group;
  }
  ...
  function createHeadAssembly(headNumber, geometry, labels) {
    ...
    const base = new THREE.Mesh(
      ...
      material({ color: active ? 0x30221e : 0x151e23, roughness: 0.30, metalness: 0.82, emissive: active ? 0x351006 : 0x000000, emissiveIntensity: active ? 0.32 : 0 })
    );
    ...
    if (active) {
      ...
      group.add(halo);
      const bottle = createBottleModel(geometry, labels);
  ...
  function createSpenderPlateAssembly(aggregateItem, geometry) {
    ...
    const arm = new THREE.Mesh(new THREE.BoxGeometry(armLength, 0.065, 0.075), armMaterial);
    arm.position.x = -armLength / 2;
    sidePivot.add(arm);
    const clampMesh = new THREE.Mesh(new THREE.CylinderGeometry(0.075, 0.075, 0.11, 28), material({ color: 0x626d72, roughness: 0.28, metalness: 0.84 }));
    clampMesh.rotation.x = Math.PI / 2;
    clampMesh.position.x = -armLength * 0.82;
    sidePivot.add(clampMesh);
    ...
    const peelEdge = new THREE.Mesh(new THREE.BoxGeometry(0.022, plateHeight * 0.98, plateThickness * 1.35), edgeMaterial);
  ...
  function createEquipmentAssembly(item) {
    ...
    if (item.kind === "roller") {
      const roller = new THREE.Mesh(new THREE.CylinderGeometry(0.055, 0.055, 0.48, 28), material({ color: 0x4e91b8, roughness: 0.62, metalness: 0.12 }));
      ...
      if (measured) {
        ...
        const support = new THREE.Mesh(new THREE.BoxGeometry(0.05, number(item?.wipePad?.heightWorld, 0.31) * 1.18, 0.075), material({ color: 0x5f6b70, metalness: 0.72, roughness: 0.33 }));
      ...
      const brush = new THREE.Mesh(new THREE.BoxGeometry(clamp(span, 0.12, 0.70), 0.42, 0.085), material({ color: 0xc3a16d, roughness: 0.88, metalness: 0.02 }));
      brush.position.y = 0.60;
      group.add(brush);
    } else if (item.kind === "coding") {
      const housing = new THREE.Mesh(new THREE.BoxGeometry(0.20, 0.25, 0.15), material({ color: 0x9b72d6, roughness: 0.40, metalness: 0.30, emissive: 0x211139, emissiveIntensity: 0.35 }));
      housing.position.y = 0.72;
      group.add(housing);
    } else {
      const generic = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.40, 0.16), material({ color: 0x778891, roughness: 0.48, metalness: 0.40 }));
  ...
  function populationSignature(snapshot) {
    ...
    return [
      ...
      labels.brand,
      ...["neck", "body", "back"].map((section) => {
        const item = labels?.sections?.[section] || {};
        return `${section}:${item.enabled}:${item.wrapDegrees}:${item.bottomWorld}:${item.topWorld}`;
      })
    ].join("|");
  }
  ...
  function rebuildHeads(snapshot) {
    ...
    for (let head = 1; head <= count; head += 1) {
      const assembly = createHeadAssembly(head, snapshot.geometry, snapshot.labels);
  ...
  function equipmentSignature(snapshot) {
    ...
    const visibleObjects = Array.isArray(equipment?.objects) ? equipment.objects.filter((item) => item?.kind !== "sensor") : [];
    return [
      equipment?.mapId,
      equipment?.mapName,
      ...
      ...visibleObjects.map((item) => [item.id,item.kind,item.angleDegrees,item.mapDepthUnits,item.spanDegrees,item.wipePad?.contactFaceRadiusWorld,item.wipePad?.heightWorld,item.wipePad?.totalThicknessWorld].join(":"))
    ].join("|");
  }
  ...
  function rebuildEquipment(snapshot) {
    ...
    const objects = Array.isArray(snapshot?.equipment?.objects) ? snapshot.equipment.objects.filter((item) => item?.kind !== "sensor") : [];
    objects.forEach((item) => {
      const assembly = createEquipmentAssembly(item);
      if (!assembly) return;
  ...
  function updateLabelVisibility(snapshot) {
    const meshes = activeBottleModel?.userData?.labelMeshes || {};
    ["neck", "body", "back"].forEach((section) => {
      if (!meshes[section]) return;
      meshes[section].visible = Boolean(snapshot?.labels?.sections?.[section]?.applied);
    });
  }
  ...
  function updateActivityHighlight(snapshot) {
    ...
    equipmentAssemblies.forEach((assembly) => {
      ...
      const active = activeStage === "wipe" && Number.isFinite(activeAggregate) && Number(assembly.userData.station) === activeAggregate;
  ...
  function updateTelemetry(snapshot) {
    ...
    const visibleObjects = Array.isArray(equipment.objects) ? equipment.objects.filter((item) => item?.kind !== "sensor").length : 0;
    const activeSections = snapshot?.labels?.activeSections || [];
    const appliedSections = snapshot?.labels?.appliedSections || [];
    ...
    ui.labels.textContent = `${snapshot?.labels?.brand || "—"} • ${appliedSections.length}/${activeSections.length} applied`;
  ...
  function applySnapshot(snapshot) {
    ...
    updateLabelVisibility(snapshot);
  ...
  function renderFrame() {
    ...
    try {
      ...
      lastSnapshot = activeRuntime.snapshot({ scene: { tableY: TABLE_Y, bottleLift: BOTTLE_LIFT, unitMode: "physical-mm-bottle-labels-v0.7" } });
  ...
  async function openViewport() {
    ...
    try {
      await ensureThree();
      if (!wipeMeshFactory()?.createMeasuredAssembly) throw new Error("Measured wipe-pad mesh factory is unavailable.");
      if (!labelMeshFactory()?.createBottleLabels) throw new Error("Bottle label mesh factory is unavailable.");
      ...
      console.error("ServoForge 3D bottle-label renderer failed", error);
    }
  }
  ...
  function status() {
    return Object.freeze({
      ...
      labelNeckHeightAuthority: Boolean(lastSnapshot?.labels?.neckHeightAuthority),
      labelMeshFactory: labelMeshFactory()?.FACTORY_VERSION || "unavailable",
      ...
      equipmentCount: Array.isArray(lastSnapshot?.equipment?.objects) ? lastSnapshot.equipment.objects.filter((item) => item?.kind !== "sensor").length : 0,
  ...
  function installWhenReady(attempt = 0) {
    if (!runtime()?.snapshot || !wipeMeshFactory()?.createMeasuredAssembly || !labelMeshFactory()?.createBottleLabels) {
