(function installServoForge3DBottleHandlingViewportRenderer(global) {
  "use strict";
  ...
  function progressiveLabelFlow() { return global.Labeler3DProgressiveLabelFlow || null; }
  function viewportControls() { return global.Labeler3DViewportUiControls || null; }
  ...
  function installUi() {
    ...
    let backdrop = document.querySelector("#servoforge3dBackdrop");
    ...
    backdrop.innerHTML = `
      <section class="servoforge-3d-panel" role="dialog" aria-modal="true" aria-labelledby="servoforge3dTitle">
        ...
        <div class="servoforge-3d-stage" id="servoforge3dStage">
          ...
          <div class="servoforge-3d-note"><strong>Scene authority:</strong> the starwheel bottle-handling environment is the only mechanical transport scene. The retired generic carousel/head environment is not created.</div>
  ...
  function equipmentSignature(snapshot) {
    ...
    return [
      ...
      ...(equipment.objects || []).map((item) => [item.id, item.kind, item.angleDegrees, item.mapDepthUnits, item.spanDegrees, item.sensorFieldOfViewDegrees, item.wipePad?.contactFaceRadiusWorld].join(":"))
    ].join("|");
  }
  ...
  async function createMachineScene() {
    scene = new THREE.Scene();
    scene.name = "ServoForgeCanonicalBottleHandlingScene";
    scene.userData.servoforgeSceneAuthority = VIEWPORT_VERSION;
    ...
    camera.name = "ServoForgeCanonical3DCamera";
    ...
    const handling = handlingViewport();
    if (!handling?.attachScene) throw new Error("Bottle-handling scene authority is unavailable.");
    await handling.attachScene(scene);
  ...
  function updateActivityHighlight(snapshot) {
    ...
    equipmentAssemblies.forEach((assembly) => {
      ...
      const active = sameStation && ((activeStage === "wipe" && (kind === "pad" || kind === "roller")) || (activeStage.includes("sensor") && kind === "sensor") || (activeStage.includes("cod") && kind === "coding"));
  ...
  function updateTelemetry(snapshot) {
    ...
    ui.hardwareCatalog.textContent = catalogStatus ? `${catalogStatus.profileCount} profiles • photo ref` : "Unavailable";
    ui.mapName.textContent = equipment.mapName || "—";
    ui.equipmentCount.textContent = `${equipment?.aggregates?.length || 0} spender • ${equipment?.objects?.length || 0} map objects`;
    ui.sensors.textContent = `${number(counts.sensors)} visible`;
    ui.coders.textContent = `${number(counts.coding)} visible`;
    ui.measuredPads.textContent = `${number(counts.measuredPads)}/${number(counts.pads)} measured`;
  ...
  function applySnapshot(snapshot) {
    ...
    progressiveLabelFlow()?.sync?.(snapshot);
    rebuildEquipment(snapshot);
    updateActivityHighlight(snapshot);
    if (followHead) focusHead(snapshot);
    updateTelemetry(snapshot);
    viewportControls()?.syncScene?.(scene, camera, snapshot);
  }

  function notifyFrameListeners(now, snapshot) {
    frameListeners.forEach((listener) => {
      try { listener({ now, snapshot, scene, camera, renderer }); }
      catch (error) { console.warn("ServoForge 3D frame listener skipped", error); }
    });
  }
  ...
  function renderFrame(now) {
    ...
    try {
      ...
      lastSnapshot = activeRuntime.snapshot({ scene: { tableY: TABLE_Y, bottleLift: BOTTLE_LIFT, unitMode: "physical-mm-starwheel-single-scene-v0.9" } });
      applySnapshot(lastSnapshot);
      notifyFrameListeners(now, lastSnapshot);
  ...
  async function openViewport() {
    ...
    try {
      ...
      if (!handlingViewport()?.attachScene) throw new Error("Starwheel bottle-handling scene authority is unavailable.");
      if (!renderer) await createMachineScene();
      else await handlingViewport().attachScene(scene);
      resizeRenderer();
      ui.error.hidden = true;
