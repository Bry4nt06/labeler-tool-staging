...
(function installIncrementalRotationProfile() {
  ...
  function profileSelected() {
    return String(state.selectedMotionProfileId || state.defaultMotionProfileId || "automatic") === PROFILE_ID;
  }
  ...
  function applyResult(result) {
    ...
    if (grammar?.annotateCorrectionChains) {
      ...
      state.machineFamilyGrammar = { family: annotated.family, rule: annotated.rule, chains: annotated.chains, chainCount: annotated.chains.length };
  ...
  function summary(result) {
    ...
    if (!result.applied) return result.fallbacks?.length
      ? result.fallbacks.map((item) => item.message).join(" ")
  ...
  function installSelectionHandler() {
    document.addEventListener("change", (event) => {
      ...
      state.selectedMotionProfileId = event.target.value;
  ...
  function install() {
    ...
    if (typeof state === "undefined" || typeof allMotionProfiles !== "function" || typeof applyGeneratedServoProfile !== "function" || !window.LabelerIncrementalRotationDriver) return false;
    installed = true;
    installProfileEntry();
