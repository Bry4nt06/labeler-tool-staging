...
(function installMessageTopology(root, factory) {
  ...
  function taskContext(project, program, routine) {
    ...
    return {
      schedules,
      taskRootReachable: Boolean(scheduling?.taskRootReachableRoutines?.includes(routineKey)),
  ...
  function buildMessageCalls(project, messageTags) {
    ...
    for (const rung of project.rungs || []) {
      for (const call of findMsgCalls(rung.source)) {
        ...
        calls.push({
          controlTag: call.controlTag,
          program: rung.program || null,
          routine: rung.routine || null,
          rung: rung.number,
          line: rung.startLine,
          source: call.raw,
          configurationResolved: Boolean(config),
          configurationScope: config?.scope || null,
          messageType: config?.messageType || null,
          direction: messageDirection(config?.messageType),
          connectionPath: config?.connectionPath || null,
          remoteElement: config?.remoteElement || null,
          localElement: config?.localElement || null,
          destinationTag: config?.destinationTag || null,
          pathModuleMatches: moduleMatchesForPath(project, config?.connectionPath),
  ...
  function buildFindings(messageTags, calls) {
    ...
    const add = (id, classification, severity, title, summary, evidence) => findings.push({
      id, classification, severity, title, summary, evidence, sourceRule: "plc-message-v9"
    });
    ...
    for (const call of calls) {
      if (!call.controlTag || !call.configurationResolved) {
        add(
          ...
          "source-proven",
          "review",
          `MSG control tag unresolved: ${call.controlTag || "not identified"}`,
          `A source-visible MSG instruction was found at ${call.program || "?"}/${call.routine || "?"} rung ${call.rung}, but its MESSAGE control-tag configuration was not resolved in the supplied export. Verify project/export completeness and scope. This does not prove a live communications fault.`,
          { call }
        );
        continue;
      }
      add(
        ...
        "source-proven",
        "info",
        `MSG instruction: ${call.controlTag}`,
        `${call.controlTag} is invoked by a source-visible MSG instruction${call.messageType ? ` configured as ${call.messageType}` : ""}${call.connectionPath ? ` with source path ${call.connectionPath}` : ""}. The L5K proves configuration and invocation only; it does not prove the rung executes, the message is sent, or the transaction succeeds.`,
        { call }
      );
    }
    ...
    for (const tag of messageTags) {
      ...
      if (!tag.messageType || direction === "unconfigured") {
        add(
          `v9:message-unconfigured:${tag.scope}:${tag.name}`,
          "source-proven",
          "review",
          `MESSAGE tag is unconfigured: ${tag.name}`,
          `${tag.name} is declared as MESSAGE in the supplied export, but MessageType is ${tag.messageType || "not present"}. This is source configuration evidence only and may be intentional during commissioning or dynamic configuration.`,
          { tag }
        );
      }
      if ((direction === "inbound-read" || direction === "outbound-write") && (!tag.remoteElement || !tag.localElement)) {
        add(
          `v9:message-elements-incomplete:${tag.scope}:${tag.name}`,
          "source-proven",
          "review",
          `MESSAGE source/destination evidence incomplete: ${tag.name}`,
          `${tag.name} is configured as ${tag.messageType || "a data-table message"}, but the supplied MESSAGE tag does not expose both LocalElement and RemoteElement. Verify the intended export/revision; this analyzer does not infer missing message operands or runtime configuration.`,
          { tag }
        );
      }
    }
  ...
  function parseL5K(input, options = {}) {
    ...
    const byControlTag = messageTags.map((tag) => ({
      ...
      direction: messageDirection(tag.messageType),
      connectionPath: tag.connectionPath,
      remoteElement: tag.remoteElement,
      localElement: tag.localElement,
      destinationTag: tag.destinationTag,
      requestedLength: tag.requestedLength,
    ...
    project.dependencies.messageTopology = {
      ...
      statistics: {
        messageTags: messageTags.length,
        msgCalls: calls.length,
        resolvedCalls: calls.filter((call) => call.configurationResolved).length,
        taskRootReachableCalls: calls.filter((call) => call.taskRootReachable).length
      }
    };
    ...
    project.statistics = {
      ...
      messageTags: messageTags.length,
      msgCalls: calls.length,
      messageFindings: findings.length,
  ...
  function traceTarget(project, target, options = {}) {
    ...
    const contexts = (topology?.messageTags || [])
      .filter((tag) => symbols.has(tag.name))
      .map((tag) => ({
        controlTag: tag.name,
        scope: tag.scope,
        messageType: tag.messageType,
        ...
        localElement: tag.localElement,
        destinationTag: tag.destinationTag,
        calls: (topology?.calls || []).filter((call) => call.controlTag === tag.name)
      }));
    return {
      ...trace,
      messageRuntimeStateProven: false,
      messageContexts: contexts,
      messageNote: contexts.length ? "MESSAGE configuration and MSG call sites are source-visible. EN/DN/ER state, actual execution, route health, completion, and payload freshness are not proven by this offline export." : null
    };
  }
  ...
  function searchAnalysis(project, query, limit = 50) {
    ...
    for (const tag of topology?.messageTags || []) {
      const haystack = JSON.stringify(tag).toLowerCase();
      if (!haystack.includes(needle)) continue;
      extra.push({
        type: "message-tag",
        key: `${tag.scope}|${tag.name}`,
        title: `${tag.name} • ${tag.messageType || "MESSAGE"}`,
        detail: `${tag.connectionPath || "path not present"} • ${tag.remoteElement || "remote element not present"}`,
        score: tag.name.toLowerCase() === needle ? 120 : 78,
        source: tag.raw
      });
    }
    for (const [index, call] of (topology?.calls || []).entries()) {
      const haystack = JSON.stringify(call).toLowerCase();
      if (!haystack.includes(needle)) continue;
      extra.push({
        type: "msg-call",
        key: String(index),
        title: `MSG ${call.controlTag || "control unresolved"}`,
        detail: `${call.program || "?"}/${call.routine || "?"} rung ${call.rung} • ${call.messageType || "configuration unresolved"}`,
        score: String(call.controlTag || "").toLowerCase() === needle ? 115 : 76,
        source: call.source
      });
    }
