...
(function installPlcImportAssistant(global) {
  ...
  function parseInWorker(text) {
    return new Promise((resolve, reject) => {
      let worker;
      try { worker = new Worker("./l5k-analyzer-worker.js?v=1"); }
      catch { worker = null; }
      if (!worker) {
        try {
  ...
  function renderCandidates() {
    ...
    els.candidateList.innerHTML = candidates.length ? candidates.map((candidate) => {
      ...
      return `<div class="plc-row plc-import-candidate" data-coverage="${esc(candidate.coverageStatus)}">
        ...
        <div class="plc-import-status-row"><span class="plc-chip">${esc(candidate.relatedTimers.length)} timers</span><span class="plc-chip">${esc(candidate.relatedCounters.length)} counters</span><span class="plc-chip">${esc(candidate.ioReferences.length)} I/O refs</span><span class="plc-chip">${esc(candidate.motionReferences.length)} motion refs</span></div>
  ...
  async function analyze() {
    ...
    try {
      ...
      setFileStatus("Parsing PLC source…", "loading");
      ...
      renderQueue();
      setFileStatus(`Review queue built from ${state.project.statistics?.rungs || 0} indexed rungs.`, "pass");
    } catch (error) {
      console.error(error);
      state.project = null;
      state.queue = null;
