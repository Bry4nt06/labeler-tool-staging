...
(function installTopModulCartReadinessIsolation(root, factory) {
  ...
  return function extendLibrary(base) {
    if (!base?.getTopModulFault || !base?.searchEntries || !base?.getFlow) {
    ...
    const stage = (order, name, signal, sourceLogic, meaning, handoff = []) => Object.freeze({ order, name, signal, sourceLogic, meaning, handoff: Object.freeze(handoff) });
    ...
    const STAGES = freezeRows([
      ...
      stage(4, "Contactor raw-state supervision", "E2001_C101_ServoPowerSupply / E2001_CB101_ServoPowerSupply / TON_MonitorMainContactor", "Fault 005 monitors equal raw C101 command/feedback states: both true OR both false, for TON_MonitorMainContactor.PRE=1500 ms.", "Do not convert the tag names into assumed NO/NC polarity. The executable source says healthy raw states are not equal during the monitored condition.", [5]),
    ...
    const PLAN = Object.freeze({
      ...
      scope: "TopModul APL Cart / shared readiness and servo-enable chain",
      source: SOURCE,
      stages: STAGES,
      ...
      sourceRefs: Object.freeze([
        ...
        "L5K 17144: Fault 005 equal raw C101 command/feedback states -> 1500 ms monitor",
    ...
    function evaluateTopModulCartReadiness(observation = {}) {
      ...
      if (o.enableMachineOn === "no" && o.enableMachineJog === "no") return outcome(
        ...
        "Use the exact active alarm stack and diagnose the earliest source-backed cart fault before opening the contactor, servo or encoder path.", UPSTREAM_ENABLE_FAULTS
      );
      ...
      if (["0", "1"].includes(o.contactorCommand) && o.contactorCommand === o.contactorFeedback) return outcome(
        "contactor-raw-state-mismatch", "direct", "Raw C101 command and feedback match — Fault 005 monitor condition",
        "The supplied Fault 005 rung treats both-true or both-false C101 command/feedback as an invalid state and supervises it for 1500 ms.",
        "Open Fault 005 and trace the exact contactor/feedback circuit only after confirming the raw states persist through the monitor window.", [5]
      );
      if (o.powerAndFeedbackOn === "yes" && o.releaseFeedbackOn === "no") return outcome(
        ...
        "Compare C101 command, raw C101 feedback, DelayMainContactorOff.DN and Fault 005 chronology without inferring physical contact polarity from the tag names.", [5]
      );
      if (o.releaseFeedbackOn === "yes" && o.readyForFeedbackOn === "no") return outcome(
        ...
        "Observe those six conditions together. Fault 016 covers GroupSynced loss; Fault 017 is the MSO feedback-on fault once ReadyForFeedbackON exists.", [16, 17]
      );
      ...
      if (o.feedbackCommand === "yes" && o.feedbackIsOn === "no") return outcome(
        ...
        "Check the MSO status/error and Fault 017 producer before replacing the encoder or changing start logic.", [17]
      );
      ...
      if (o.stationAutoOn === "yes" && o.baseReady === "no") return outcome(
        ...
        "Observe the exact DataFromLS ready rung as written; preserve raw E1501_P133_CarriageFront polarity and use Fault 020 only when its own producer is active.", [20]
      );
      ...
      return outcome(
        ...
        "Start at the first false stage and use its source-backed handoff rather than skipping downstream to servo/encoder replacement.", []
      );
    }
    ...
    const ENTRY = Object.freeze({
      ...
      title: "APL Cart will not become ready / will not start",
      summary: "Follow the supplied APL Cart PLC sequence from the Control/ControlOut enable snapshot through PowerAndFeedbackOn, C101 contactor supervision, axis feedback enable, StationAutoOn and the ready outputs. This cross-routine route intentionally reuses the existing exact fault families instead of blaming the servo/encoder whenever the cart is not ready.",
      probableCauses: Object.freeze([
        ...
        "C101 command/feedback raw states are in the Fault 005 monitored combination, or ReleaseFeedbackOn is not made.",
      ...
      actions: Object.freeze([
        ...
        "If an exact numbered alarm is present, use that fault's existing drill-down and alarm chronology as stronger evidence than this generic readiness symptom route."
      ]),
      safety: SAFETY,
      sourceRefs: Object.freeze([{ sourceId: "lb1-aplcart-1", locator: "Readable export CO85_LB1_APLCart_1.L5K — Initial, StartStop, ServoMainDrive, FaultDetection and DataToLS routines" }]),
      processTrace: Object.freeze({
        ...
        source: SOURCE,
        hardwareSource: null,
        summary: "Cross-routine APL Cart readiness chain from enable snapshot to base-machine readiness output.",
        safetyBoundary: SAFETY.join(" "),
        scopeNote: "This is a source-backed symptom route, not a new PLC fault number or invented HMI alarm. Existing numbered fault modules remain authoritative for their own producers."
      })
    });
    ...
    const FLOW = Object.freeze({
      ...
      nodes: Object.freeze({
        "cart-alarm": Object.freeze({ question: "Do you have an exact Cart/station alarm from the event?", help: "Exact alarm chronology outranks a generic not-ready symptom.", choices: Object.freeze([
          Object.freeze({ label: "Yes — use the exact fault first", action: "search", hint: "Search the fault number or use the alarm-stack analyzer." }),
        ...
        "cart-enable": Object.freeze({ question: "Are ControlOut.EnableMachineOn or ControlOut.EnableMachineJog present?", help: "Observe only; do not force the Control or ControlOut structures.", choices: Object.freeze([
          Object.freeze({ label: "No — both are absent", result: ENTRY.id, hint: "Stay upstream: identify the fault/inhibit that removed the cart enable." }),
        ...
        "cart-release-feedback": Object.freeze({ question: "Does StartStop.ReleaseFeedbackOn turn on?", help: "This separates the C101 contactor/raw feedback sequence from the motion-axis readiness conditions.", choices: Object.freeze([
          Object.freeze({ label: "No", result: ENTRY.id, hint: "Compare C101 command/feedback and Fault 005 timing." }),
        ...
        "cart-feedback-on": Object.freeze({ question: "Does MainDrive.SData.Status.Feedback_Is_ON become true?", help: "ReadyForFeedbackON must pass the 750 ms command delay, MSO, then DriveEnableStatus/20 ms feedback state.", choices: Object.freeze([
          Object.freeze({ label: "No", result: ENTRY.id, hint: "Use the MSO / Fault 017 handoff." }),
    ...
    function validate() {
      ...
      if (!STAGES.some((row) => /TON_MonitorMainContactor/.test(row.signal) && /1500 ms/.test(row.sourceLogic))) errors.push("v355 lost Fault 005 1500 ms raw-state monitor.");
      ...
      if (base.getTopModulFault(5)?.title !== "Main Contactor fault") errors.push("v355 must preserve existing Fault 005 authority.");
      if (base.getTopModulFault(16)?.title !== "Motion group not synchronized") errors.push("v355 must preserve existing Fault 016 authority.");
      if (base.getTopModulFault(17)?.title !== "Feedback on fault main drive") errors.push("v355 must preserve existing Fault 017 authority.");
