(function installServoForge3DMachineMapViewport(global) {
  "use strict";

  const VIEWPORT_VERSION = "servoforge.3d-viewport.v0.4";
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .servoforge-3d-telemetry{position:absolute;left:14px;bottom:14px;display:grid;grid-template-columns:repeat(5,minmax(112px,1fr));gap:8px;width:min(1130px,calc(100% - 28px));pointer-events:none}.servoforge-3d-metric{background:rgba(5,15,20,.85);border:1px solid rgba(137,174,190,.2);border-radius:10px;padding:8px 9px;box-shadow:0 8px 24px rgba(0,0,0,.2);min-width:0}.servoforge-3d-metric span{display:block;color:#829ba7;font-size:9px;text-transform:uppercase;letter-spacing:.08em}.servoforge-3d-metric strong{display:block;margin-top:3px;font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.servoforge-3d-metric.action{grid-column:span 2}.servoforge-3d-metric.physical strong{color:#b9f4d8}.servoforge-3d-metric.live strong{color:#ffad8f}.servoforge-3d-metric.map strong{color:#b9ddff}
      .servoforge-3d-note{position:absolute;right:14px;top:14px;max-width:390px;background:rgba(5,15,20,.80);border:1px solid rgba(137,174,190,.16);border-radius:9px;padding:8px 10px;color:#9db2bb;font-size:10px;line-height:1.42;pointer-events:none}.servoforge-3d-note strong{color:#ffd1c1}
      .servoforge-3d-legend{position:absolute;left:14px;top:14px;background:rgba(5,15,20,.78);border:1px solid rgba(137,174,190,.16);border-radius:9px;padding:8px 10px;color:#9db2bb;font-size:10px;line-height:1.5;pointer-events:none}.servoforge-3d-legend b{color:#eef7fa}.servoforge-3d-help{position:absolute;right:14px;bottom:14px;background:rgba(5,15,20,.72);border:1px solid rgba(137,174,190,.16);border-radius:9px;padding:8px 10px;color:#8ca3ad;font-size:10px;pointer-events:none}
  ...
  function installUi() {
    ...
    backdrop.innerHTML = `
      <section class="servoforge-3d-panel" role="dialog" aria-modal="true" aria-labelledby="servoforge3dTitle">
        <header class="servoforge-3d-head">
          <div class="servoforge-3d-title"><span class="servoforge-3d-live" aria-hidden="true"></span><div><h2 id="servoforge3dTitle">ServoForge 3D • Machine Map Equipment</h2><small>Measured bottle-table geometry • active machine-map equipment • read only • Three.js ${THREE_VERSION}</small></div></div>
        ...
        <div class="servoforge-3d-stage" id="servoforge3dStage">
          <canvas class="servoforge-3d-canvas" id="servoforge3dCanvas" aria-label="Live ServoForge 3D machine map and bottle-table simulation"></canvas>
          <div class="servoforge-3d-note"><strong>Equipment placement:</strong> angular locations come directly from the active machine map. Radial stand-off and housing dimensions are map-derived approximations until physical measurements/CAD are supplied.</div>
          <div class="servoforge-3d-legend"><b>Map equipment</b><br>Orange: live Head 1<br>Steel: aggregates<br>Blue: wipe rollers<br>Tan: wipe pads<br>Cyan: sensors<br>Purple: coder</div>
          <div class="servoforge-3d-telemetry" aria-live="polite">
            ...
            <div class="servoforge-3d-metric map"><span>Map equipment</span><strong id="servoforge3dEquipmentCount">—</strong></div>
            <div class="servoforge-3d-metric map"><span>Active aggregate</span><strong id="servoforge3dAggregate">—</strong></div>
            <div class="servoforge-3d-metric physical"><span>Physical pitch radius</span><strong id="servoforge3dPitchRadius">—</strong></div>
    ...
    ui = {
      ...
      aggregate: backdrop.querySelector("#servoforge3dAggregate"),
  ...
  function equipmentColor(kind) {
    if (kind === "roller") return 0x4e91b8;
    if (kind === "pad" || kind === "brush" || kind === "brush-channel") return 0xc3a16d;
    if (kind === "sensor") return 0x4fd7ef;
    if (kind === "coding") return 0x9b72d6;
    if (kind === "gripper") return 0x7cbd75;
    return 0x778891;
  }

  function createAggregateAssembly(item) {
    const group = new THREE.Group();
    group.name = `ServoForgeAggregate${item.aggregate}`;
    group.userData.aggregate = item.aggregate;
    const postMaterial = material({ color: 0x49565d, roughness: 0.34, metalness: 0.68, emissive: 0x000000, emissiveIntensity: 0 });
    const post = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.95, 0.18), postMaterial);
    post.position.y = 0.48;
    group.add(post);
    const headMaterial = material({ color: 0x68777e, roughness: 0.30, metalness: 0.72, emissive: 0x000000, emissiveIntensity: 0 });
    const head = new THREE.Mesh(new THREE.BoxGeometry(0.52, 0.22, 0.34), headMaterial);
    head.position.set(-0.20, 0.84, 0);
    group.add(head);
    const nose = new THREE.Mesh(new THREE.BoxGeometry(0.22, 0.12, 0.20), material({ color: 0xb6c0c4, roughness: 0.28, metalness: 0.74 }));
    nose.position.set(-0.52, 0.80, 0);
    group.add(nose);
    group.position.set(number(item.position?.x), 0, number(item.position?.z));
    group.rotation.y = number(item.rotationY);
    group.userData.highlightMaterials = [postMaterial, headMaterial];
    return group;
  }
  ...
  function createEquipmentAssembly(item, snapshot) {
    ...
    if (item.kind === "roller") {
      const roller = new THREE.Mesh(
        ...
        material({ color, roughness: 0.62, metalness: 0.12 })
      );
      roller.position.y = 0.57;
      group.add(roller);
      const shaft = new THREE.Mesh(new THREE.CylinderGeometry(0.018, 0.018, 0.58, 20), material({ color: 0xaeb9be, metalness: 0.78, roughness: 0.28 }));
      shaft.position.y = 0.57;
      group.add(shaft);
    } else if (item.kind === "pad" || item.kind === "brush" || item.kind === "brush-channel") {
      const pad = new THREE.Mesh(
        new THREE.BoxGeometry(clamp(span, 0.12, 0.70), 0.42, 0.085),
        material({ color, roughness: 0.88, metalness: 0.02 })
      );
      pad.position.y = 0.60;
      group.add(pad);
      const bracket = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.54, 0.06), material({ color: 0x69767c, metalness: 0.70, roughness: 0.34 }));
      bracket.position.set(span * 0.38, 0.54, 0.08);
      group.add(bracket);
    } else if (item.kind === "sensor" || item.kind === "coding") {
      const housing = new THREE.Mesh(
        new THREE.BoxGeometry(item.kind === "coding" ? 0.20 : 0.14, item.kind === "coding" ? 0.25 : 0.15, 0.15),
        material({ color, roughness: 0.40, metalness: 0.30, emissive: item.kind === "sensor" ? 0x063d4a : 0x211139, emissiveIntensity: 0.35 })
      );
      housing.position.y = 0.72;
      group.add(housing);
      const lens = new THREE.Mesh(new THREE.CircleGeometry(0.028, 24), material({ color: 0xe8fbff, emissive: color, emissiveIntensity: 0.8, roughness: 0.25 }));
      lens.rotation.y = Math.PI / 2;
      lens.position.set(-0.071, 0.72, 0);
      group.add(lens);
    } else {
      const generic = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.40, 0.16), material({ color, roughness: 0.48, metalness: 0.40 }));
    ...
    if (item.kind === "sensor" || item.kind === "coding") {
      const pitchRadius = number(snapshot?.equipment?.physicalPitchRadiusWorld, number(snapshot?.geometry?.machine?.physicalPitchRadiusWorld, 3.5));
      const x = number(item.position?.x);
      const z = number(item.position?.z);
      const length = Math.hypot(x, z) || 1;
      const targetX = x / length * pitchRadius;
      const targetZ = z / length * pitchRadius;
      const beamGeometry = new THREE.BufferGeometry().setFromPoints([
        new THREE.Vector3(x, 0.72, z),
        new THREE.Vector3(targetX, 0.72, targetZ)
      ]);
      const beam = new THREE.Line(
        beamGeometry,
        new THREE.LineBasicMaterial({ color, transparent: true, opacity: item.kind === "sensor" ? 0.72 : 0.55 })
      );
      beam.name = `ServoForgeBeam-${item.id}`;
      equipmentLayer.add(beam);
  ...
  function clearEquipment() {
    equipmentAssemblies.forEach((assembly) => {
      if (assembly.userData?.beam) {
        equipmentLayer?.remove(assembly.userData.beam);
        disposeObject3D(assembly.userData.beam);
      }
  ...
  function populationSignature(snapshot) {
    ...
    return [snapshot?.carousel?.headCount, geometry?.bottle?.bottleType, geometry?.bottle?.effectiveDiameterMm, geometry?.bottle?.referenceHeightMm, geometry?.bottleTable?.plateDiameterWorld].join("|");
  }
  ...
  function equipmentSignature(snapshot) {
    ...
    return [
      ...
      ...(Array.isArray(equipment?.objects) ? equipment.objects.map((item) => `${item.id}:${item.kind}:${item.angleDegrees}:${item.mapDepthUnits}:${item.spanDegrees}`) : [])
    ].join("|");
  }
  ...
  function rebuildEquipment(snapshot) {
    ...
    aggregates.forEach((item) => {
      const assembly = createAggregateAssembly(item);
      aggregateAssemblies.push(assembly);
      aggregateLayer.add(assembly);
    });
    const objects = Array.isArray(snapshot?.equipment?.objects) ? snapshot.equipment.objects : [];
  ...
  function createMachineScene() {
    ...
    const floor = new THREE.Mesh(new THREE.CircleGeometry(8.5, 96), material({ color: 0x0b171d, roughness: 0.86, metalness: 0.05 }));
    ...
    carouselBody = new THREE.Mesh(new THREE.CylinderGeometry(1, 1.035, 0.20, 96), material({ color: 0x202b31, roughness: 0.42, metalness: 0.72 }));
    carouselBody.position.y = 0.10;
    carouselBody.receiveShadow = true;
    scene.add(carouselBody);
    carouselTop = new THREE.Mesh(new THREE.CylinderGeometry(1, 1, 0.035, 96), material({ color: 0x35434a, roughness: 0.34, metalness: 0.78 }));
    carouselTop.position.y = 0.218;
    scene.add(carouselTop);
    pathRing = new THREE.Mesh(new THREE.TorusGeometry(1, 0.024, 10, 160), material({ color: 0xff6a3d, emissive: 0x4a1408, emissiveIntensity: 0.42, roughness: 0.35 }));
    pathRing.rotation.x = Math.PI / 2;
    pathRing.position.y = 0.247;
    scene.add(pathRing);
    hub = new THREE.Mesh(new THREE.CylinderGeometry(0.72, 0.82, 0.34, 64), material({ color: 0x10191e, roughness: 0.3, metalness: 0.82 }));
    hub.position.y = 0.34;
    scene.add(hub);
    ...
    aggregateLayer.name = "ServoForgeAggregateEquipment";
    scene.add(aggregateLayer);
    equipmentLayer = new THREE.Group();
    equipmentLayer.name = "ServoForgeMachineMapObjects";
  ...
  function updateAggregateHighlight(snapshot) {
    ...
    aggregateAssemblies.forEach((assembly) => {
      ...
      (assembly.userData.highlightMaterials || []).forEach((entry) => {
        entry.emissive?.setHex?.(active ? 0x5a1d0b : 0x000000);
        entry.emissiveIntensity = active ? 0.62 : 0;
      });
    });
  }
  ...
  function updateTelemetry(snapshot) {
    ...
    const objectTotal = Object.values(counts).reduce((sum, value) => sum + number(value), 0);
    ...
    ui.equipmentCount.textContent = `${equipment?.aggregates?.length || 0} agg • ${objectTotal} objects`;
    ui.aggregate.textContent = Number.isFinite(Number(activity.aggregate)) ? `Aggregate ${activity.aggregate}` : "—";
  ...
  function applySnapshot(snapshot) {
    ...
    updateAggregateHighlight(snapshot);
  ...
  function renderFrame() {
    ...
    try {
      ...
      lastSnapshot = activeRuntime.snapshot({ scene: { tableY: TABLE_Y, bottleLift: BOTTLE_LIFT, unitMode: "physical-mm-machine-map-v0.4" } });
  ...
  async function openViewport() {
    ...
    try {
      ...
      console.error("ServoForge 3D machine-map renderer failed", error);
    }
  }
  ...
  function status() {
    return Object.freeze({
      ...
      equipmentCount: lastSnapshot?.equipment?.objects?.length || 0,
      aggregateCount: lastSnapshot?.equipment?.aggregates?.length || 0,
      equipmentAngularAuthority: "active-machine-map",
      equipmentRadialAuthority: "map-derived-not-cad",
