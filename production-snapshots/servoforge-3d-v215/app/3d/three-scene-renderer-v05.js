(function installServoForge3DMeasuredWipeViewport(global) {
  "use strict";

  const VIEWPORT_VERSION = "servoforge.3d-viewport.v0.5";
    ...
    backdrop.innerHTML = `
      <section class="servoforge-3d-panel" role="dialog" aria-modal="true" aria-labelledby="servoforge3dTitle">
        <header class="servoforge-3d-head">
          <div class="servoforge-3d-title"><span class="servoforge-3d-live" aria-hidden="true"></span><div><h2 id="servoforge3dTitle">ServoForge 3D • Measured Wipe Pads</h2><small>Measured bottle-table geometry • measured wipe-pad contact • read only • Three.js ${THREE_VERSION}</small></div></div>
        ...
        <div class="servoforge-3d-stage" id="servoforge3dStage">
          <canvas class="servoforge-3d-canvas" id="servoforge3dCanvas" aria-label="Live ServoForge 3D measured wipe-pad and bottle-table simulation"></canvas>
          <div class="servoforge-3d-note"><strong>Wipe-pad authority:</strong> 70 mm height, 18 mm sponge, 4 mm steel backing, 22 mm total stack, and 2 mm bottle penetration are measured. Pad curvature/length follows the active wipe start/stop angles. Vertical mounting center remains a reference until its machine height is measured.</div>
          <div class="servoforge-3d-legend"><b>3D equipment</b><br>Orange: live Head 1<br>Steel: aggregates / pad backing<br>Blue: wipe rollers<br>Brown: measured sponge<br>Cyan: sensors<br>Purple: coder</div>
  ...
  function createEquipmentAssembly(item, snapshot) {
    ...
    if (item.kind === "roller") {
      ...
      if (measured) {
        ...
        // Mounting support is intentionally a visual reference only; its dimensions
        // are not CAD-authoritative yet. The sponge/backing geometry above is measured.
        ...
        const fallback = new THREE.Mesh(
          ...
          material({ color: 0xb96d38, roughness: 0.92, metalness: 0.01 })
        );
        fallback.position.y = PAD_REFERENCE_CENTER_Y;
        group.add(fallback);
      }
      ...
      const lens = new THREE.Mesh(
        ...
        material({ color: 0xe8fbff, emissive: color, emissiveIntensity: 0.8, roughness: 0.25 })
      );
      ...
      const generic = new THREE.Mesh(
        ...
        material({ color, roughness: 0.48, metalness: 0.40 })
      );
    ...
    if (measuredPad) {
      // The measured annular geometry is authored with local +X pointing radially
      // away from the carousel. Three.js Y rotation uses the opposite sign for
      // this XZ map convention, so derive orientation directly from world position.
  ...
  function equipmentSignature(snapshot) {
    ...
    return [
      ...
      ...(Array.isArray(equipment?.objects) ? equipment.objects.map((item) => [
        item.id,
        item.kind,
        item.angleDegrees,
        item.mapDepthUnits,
        ...
        item.wipePad?.bottlePenetrationMm
      ].join(":")) : [])
    ].join("|");
  }
  ...
  function updateTelemetry(snapshot) {
    ...
    const objectTotal = number(counts.pads) + number(counts.rollers) + number(counts.sensors) + number(counts.coding) + number(counts.other);
    ...
    try {
      ...
      lastSnapshot = activeRuntime.snapshot({
        scene: {
          ...
          unitMode: "physical-mm-measured-wipe-v0.5"
        }
      });
  ...
  async function openViewport() {
    ...
    try {
      ...
      console.error("ServoForge 3D measured-wipe renderer failed", error);
    }
  }
  ...
  function status() {
    return Object.freeze({
      ...
      equipmentRadialAuthority: "measured-wipe-contact-plus-map-derived-other-equipment",
