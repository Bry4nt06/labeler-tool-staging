...
BUILD = "pseudo-3d-bottle-side-view-v81-20260811-1838"
UPDATED = "Aug 11, 2026 6:38 PM ET"

panel_path = Path("app/bottle-orientation-panel-integration.js")
source = panel_path.read_text()
source = source.replace("const VERSION = 5;", "const VERSION = 6;", 1)

new_side = r'''  function sideViewSvg(context) {
...
pattern = re.compile(r"  function sideViewSvg\(context\) \{.*?\n  function panelMarkup\(source\) \{", re.S)
if not pattern.search(source):
    raise SystemExit("Could not locate sideViewSvg block")
source = pattern.sub(new_side + "  function panelMarkup(source) {", source, count=1)
...
manifest["notes"] = (
    "Staging v81 upgrades the Bottle Orientation Side View to a pseudo-3D clear-glass bottle. "
    "Glass highlights, cylindrical depth cues, front/rear datum visibility, and curved clipped "
    "label-band projection now make bottle orientation easier to read while preserving the exact "
    "v80 world-frame Top View and generated servo/HMI geometry."
)
manifest_path.write_text(json.dumps(manifest, indent=2) + "\n")

test = Path("tests/bottle-orientation-panel.test.js")
text = test.read_text()
needle = 'assert.match(source, /oppositeCarouselBottleSpinV79: true/);'
addition = (
    needle
...
if needle not in text:
    raise SystemExit("Could not locate v79 assertion anchor")
text = text.replace(needle, addition, 1)
text = re.sub(
    r'assert\.match\(bootstrap, /[^/]+/\);\nassert\.match\(serviceWorker, /[^/]+/\);\nassert\.equal\(manifest\.buildId, "[^"]+"\);',
    f'assert.match(bootstrap, /{re.escape(BUILD)}/);\nassert.match(serviceWorker, /{re.escape(BUILD)}/);\nassert.equal(manifest.buildId, "{BUILD}");',
    text,
    count=1,
)
text = re.sub(
    r'console\.log\("[^"]*regression passed\."\);',
    'console.log("Pseudo-3D bottle side-view v81 regression passed.");',
    text,
    count=1,
)
