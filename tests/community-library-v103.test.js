...
assert.ok(integration.includes('id = "communityZoneFilter"'), "Zone browse filter must exist.");
assert.ok(integration.includes('Zone: All'), "Zone filter must include All.");
assert.ok(integration.includes('id = "communitySiteFilter"'), "Site browse filter must exist.");
...
assert.ok(integration.includes('.toUpperCase()'), "Community location codes must normalize to uppercase.");
assert.ok(integration.includes('replace(/[^A-Z0-9]/g, "")'), "Community location codes must stay alphanumeric.");
...
assert.ok(integration.includes('form.addEventListener("submit", submitUpload, true)'), "v103 upload handler must run in capture mode before the legacy current-selection submit handler.");
