...
test("v7.1 task Compare layer remains local/read-only under the current v10.1 release", () => {
  ...
  assert.ok(taskParser >= 0 && taskParser < messageParser && messageParser < consistencyParser && consistencyParser < baseCompare && baseCompare < dependencyCompare && dependencyCompare < taskCompare && taskCompare < communicationCompare && communicationCompare < messageCompare && messageCompare < consistencyCompare && consistencyCompare < uiScript);
