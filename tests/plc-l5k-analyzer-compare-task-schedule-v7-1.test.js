...
test("v7.1 Compare page loads task parser/comparison locally and exposes task filter", () => {
  ...
  const uiScript = page.indexOf("plc-analyzer-compare-v5-2.js?v=7.1");
  assert.ok(taskParser >= 0 && taskParser < baseCompare && baseCompare < dependencyCompare && dependencyCompare < taskCompare && taskCompare < uiScript);
  assert.match(page, /PLC ANALYZER COMPARE v7\.1 — TASK SCHEDULE DIFF/);
  assert.match(page, /value="tasks"/);
  assert.match(page, /TASK attributes, program assignment, and declared order/i);
  assert.match(ui, /new Worker\("\.\/l5k-analyzer-compare-worker\.js\?v=7\.1"\)/);
