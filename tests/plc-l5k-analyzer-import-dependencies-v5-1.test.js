...
test("v5.1 dependency import remains loaded as later Import Assistant phases advance", () => {
  ...
  assert.ok(Number(versionMatch[1]) >= 5.1, `Expected Import Assistant v5.1 or later, found v${versionMatch[1]}.`);
  assert.match(page, /DEPENDENCY EVIDENCE/);
  ...
  assert.match(page, /uploaded controller's actual tags, fault targets and source locations/i);
});
