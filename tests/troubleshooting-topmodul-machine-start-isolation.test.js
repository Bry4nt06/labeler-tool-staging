...
test("v352 machine-start layer validates through the current v351+ runtime", () => {
...
test("Fault 699 is active internal evidence without inflating the operator-named alarm count", () => {
  ...
  assert.ok(!entry.sourceRefs.some((ref) => /operator-facing.*Fault 699/i.test(ref.locator)));
});

test("Fault 699 retains the exact PE171 watchdog producer without inventing engineering units", () => {
  const entry = library.getTopModulFault(699);
...
test("Fault 699 binds PE171 to the exact K407039 page 118 input", () => {
  const entry = library.getTopModulFault(699);
...
test("Fault 699 related faults remain context evidence rather than asserted causes", () => {
...
test("alarm-stack analyzer ranks active Fault 699 as direct producer evidence", () => {
...
test("no-start symptom search resolves one shared StartStop isolation method", () => {
...
test("no-start method preserves the Control to ControlOut scan snapshot instead of treating it as a forced live bit", () => {
...
test("guided no-start flow isolates MachineOn, drive enable and C102 without duplicating fault procedures", () => {
...
test("v352 leaves 00067, base Fault 067 and main Labeler Fault 670 separated", () => {
...
test("browser loads v352 after v345 and before live bridge/alarm-stack layers", () => {
  ...
  assert.ok(match && Number(match[1]) >= 352);
});
